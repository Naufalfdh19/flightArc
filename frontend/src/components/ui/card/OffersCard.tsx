...
export default function OfferCard({
    ...
    const cardTitleSize = titleSize ? titleSizeClasses[titleSize as keyof typeof titleSizeClasses] : "text-2xl";
    ...
    return (
        <>  
            ...
            >
                ...
                <p className={`mb-3 text-white/40`}>{eyebrow}</p>
                <h1 className={`mb-3 text-white ${cardTitleSize}`}>{title}</h1>
                <p className={`mb-4 text-white/40`}>{description}</p>
                <Button square="full">{cta}</Button>
            </div>
        </>
    )
}